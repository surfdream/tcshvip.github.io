define(function (require, exports, module) {
    ...
    var create_kinds_box = juicer([
        '<div class="user_kinds">',
            ...
            '<ul class="kind_list">',
                '{@each list as item }',
                '<li class="kind_item" data_id="${item.id}"><input type="checkbox" class="ci_chk" id="ci_${collectid}_${item.id}" /><label for="ci_${collectid}_${item.id}">${item.tagname}<span class="tag_con" data_con="${item.size}">(${item.size})</span></label></li>',
                '{@/each}',
            '</ul>',
            '<div class="create_do"><label class="create_btn" for="create_txt_${collectid}"><span class="iconfont">&#xf0175;</span>新建标签</label><div class="create_kinds_box"><input type="text" class="create_txt" wmv="empty|max:10" wmvmsg="分类名不能为空！|分类名过长！" id="create_txt_${collectid}" /><a href="#" class="create_submit">创建</a></div></div>',
        '</div>'
    ].join(''));
    var init = function () {
        ////分页
    ...
    var bind = function () {
        ...
        $page.on("click", ".chk_all", function () {
            ...
            ;
            if (_checked) {
                $favorite_goods_list.find(".chk_item").attr("checked", "checked");
            }
            else {
                $favorite_goods_list.find(".chk_item").removeAttr("checked");
            }
        });
        //监听全选
        $page.find(".chk_item").on("click", function () {
            if ($favorite_goods_list.find(".chk_item:checked").length !== $favorite_goods_list.find(".chk_item").length) {
        ...
        $page.on("click", ".select_kinds", function () {
            var $this = $(this),
                $parent = $this.closest(".favorite_goods_con"),
                _collectid = $parent.attr("data_collectid")
            ;
            box.relyBox({
                rely: $this,
                boxCls: "user_kinds_box",
                content: create_kinds_box.render({ list: _g_data.userTag, collectid: _collectid }),
                btns: [],
                offset: {
                    top: -20
                },
                callback: function () {
                    ...
                    this.wmBox.on("click", ".create_submit", function () {
                        ...
                        if (verification.verify($create_kinds_box)) {
                            ...
                            collect.createCommodityTag({
                                tagName: postData.name,
                                success: function (data) {
                                    var $append;
                                    if (data.success) {
                                        $kind_list.scrollTop(0);
                                        $append = $('<li class="kind_item" data_id="' + data.success.id + '"><input type="checkbox" class="ci_chk" id="ci_' + _collectid + '_' + data.success.id + '" /><label for="ci_' + _collectid + '_' + data.success.id + '">' + data.success.tagName + '<span class="tag_con" data_con="1">(1)</span></label></li>');
                                        ...
                                        bindCiChk.call($append, _collectid);
                    ...
                    bindCiChk.call(this.wmBox, _collectid);
                }
            });
            return false;
        });
        //批量分类
        $page.on("click", ".setkind_commodity_list", function () {
            var $this = $(this),
               $checked_item = $favorite_goods_list.find(".chk_item:checked"),
            ...
            if ($checked_item.length) {
                $checked_item.each(function () {
                    _arr.push($(this).closest(".favorite_goods_con").attr("data_collectid"));
                });
                _collectid = _arr.join('_');
                box.relyBox({
                    rely: $this,
                    boxCls: "user_kinds_box",
                    content: create_kinds_box.render({ list: _g_data.userTag, collectid: _collectid }),
        ...
        //删除
        $page.on("click", ".del_commodity", function () {
            var $this = $(this),
                $parent = $this.closest(".favorite_goods_con"),
        ...
        //批量删除
        $page.on("click", ".del_commodity_list", function () {
            var $this = $(this),
                $checked_item = $favorite_goods_list.find(".chk_item:checked"),
                thisError = $this.data("thisError"),
            ...
            if ($checked_item.length) {
                if (!thisConfirm) {
                    thisConfirm = box.relyBox({
                        rely: $this,
                        content: '<p style="font-size: 14px;color: #666;line-height: 40px;"><i class="iconfont" style="color: #f37e15;font-size: 32px;line-height: 40px;vertical-align: bottom;margin-right: 10px;">&#xf0143;</i>确定掉删除选中的收藏吗？</p><span style="padding-left: 42px;color: #999;">删除后将无法恢复</span><div style="padding-left: 42px;margin: 10px 0 -10px 0;"><a href="#" style="background: #f40;padding: 5px 10px;color: #fff;margin-right: 10px;" class="close">取消</a><a href="#" class="sure">确定</a></div>',
                        callback: function () {
                            var self = this;
                            this.close = this.hide;
                            this.wmBox.hover(function () { }, function () {
                        ...
                        sureCallBack: function () {
                            var _ids = [];
                            $checked_item.each(function () {
                                _ids.push($(this).closest(".favorite_goods_con").attr("data_collectid"));
                            });
                            collect.delCommodity({
                                id: _ids.join(','),
                                success: function (data) {
                                    if (data.success) {
                                        $checked_item.each(function () {
                                            var $parent = $(this).closest(".favorite_goods_con");
                                            $parent.fadeOut(function () {
                                                $parent.remove();
                                            });
                                        });
        ...
        $page.on("click", ".shopping_cart", function () {
            var $this = $(this), $shoppingCartBox = $this.data("shoppingCartBox");
            if (!$shoppingCartBox) {
                commodity_data.getSpecification({
                    success: function () {
                        var self = this;
                        $shoppingCartBox = box.alert({
                            boxCls: 'shopping_cart_box',
                            titleText: '选择属性',
                            content: self.$buy_attr_box,
                            callback: function () {
                                this.close = this.hide;
                                this.mask.css({
                                    opacity: 0
                                ...
                                this.show();
                                $this.data("shoppingCartBox", this);
                            },
                            btns: [
                                {
                                    cls: 'ui_btn_h33yellow4',
                                    res: 'to_shopping_cart',
                                    text: '加入购物车',
                                    callback: function () {
                                        alert("正在开发中...");
                                        self.close();
                        ...
                        collect.delCommodityTag({
                            ...
                            success: function (data) {
                                if (data.success) {
                                    ...
                                    $page.find(".favorite_goods_list").empty();
                                } else {
                                    alert("系统繁忙，请稍后再试！");
                                }
                            },
        ...
        $page.on("click", ".editor_mod", function () {
            ...
            if (!_thisBox) {
                _thisBox = box.invBox({
                    offset: { top: -20 },
                    boxId: "edit_tag",
                    content: '<div class="edit_tag_con"><h3>编辑标签</h3><p style="padding:20px 80px 10px 0;"><label for="">名称：</label><input type="text" class="editor_kinds" value="" /></p><a href="#" class="submit">保存修改</a><a href="#" class="iconfont close">&#xf00b3;</a></div>',
                    callback: function () {
                        var self = this;
                        this.close = this.hide;
